...
class _CreateTOState extends State<CreateTO>
  ...
  Future<void> _updateTOInDatabase() async {
    ...
    print(
      'DEBUG: updateTOInDatabase -> ${updated.maTO}, status=${updated.trangThai}, complete=${updated.completeTime}, qty=${updated.soLuongDonHang}, w=${updated.totalWeight}',
    );
  ...
  Future<void> _processCode(String code, String codeType) async {
    ...
    print('📦 Tìm kiếm mã trong database...');
    final order = await OrderDatabase.instance.getOrder(code);

    if (order == null) {
      _showCenterMessage('Không tìm thấy đơn hàng này!', Colors.red);
      await _playErrorSound();
      print('❌ Lý do: Mã không tồn tại trong database (order_db.db hoặc orders.db)');
      print('💡 Kiểm tra: Mã này có trong bảng "orders" không?');
      return;
    }

    print('✅ Tìm thấy trong database!');
    print('📋 Chi tiết order: $order');

    final orderStation = (order['station'] ?? '').toString().trim();
    double itemWeight = (order['weight'] is num)
